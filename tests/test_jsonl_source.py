...
from src.sources.jsonl_source import JSONLinesSource, Task, JSONLinesError
...
def test_jsonl_source(test_file):
    ...
    assert isinstance(JSONLinesSource.create_source(test_file), JSONLinesSource)


def test_parser_with_error():
    with pytest.raises(JSONLinesError):
        parser("YTERWRTRYTJ", 1, "file.txt")
